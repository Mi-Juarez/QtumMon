...
reward. Your wallet will choose one or more UTXOs (previous discrete transaction
...
and "staking" refers generally to the ability or capability of the wallet to be
chosen for a block reward.
...
Overlapping reward periods (depends on your weight) would show up as
a stake increase while there was already a stake in place. In the example below,
...
Stake	Block	Relative	
 ...
 307.6  26977   500	
   ...
   0.8  27485  1007
   0.4  27486  1008
   0.0  27487  1009    All payments matured
   0.0  27488  1010
...
000,unix time, date time, balance, stake, block, weight, net weight, immature, connections, staking, expectedHours, secondsLastBlock
...
function send_email()
    send if doNotDisturb = False and send queued messages
...
qtumdIsRunning = True  # set this on startup
...
sendLowConnectionsAlert = True # send an alert for low connections
...
config_file_name = '/home/wallstreet/qtum-wallet/bin/qtumMonConfig.txt' # name of configuration file
...
delayDateHours = 3.86   # wet on 10/13/2017, delay in seconds for the loop that detects date and hour change
loopSeq = ""            # log timing of loops, if miss block
savedLoopSeq = ""       # for logging

                        # labels for the display and log
labelRow = "unix time  | date       time     |    balance  |   stake  |    block  |     weight  | net weight | immature  | con | stking |  exphrs | secLast"
logLabels = '000,unix time, date time, balance, stake, block, weight, net weight, immature, connections, staking, expectedHours, secondsLastBlock'
...
# doNotDisturb[] gives the hours when emails will not be sent (assuming they could be texts). 
...
domesticAddress = "lklklklklklklk789789@me.com"
...
internationalAddress = "ljgghgffhgfhg@wfdffd.de"
...
while True:  # wait here if qtumd is not running
    ...
    try:
        block = int(subprocess.check_output('/home/wallstreet/qtum-wallet/bin/qtum-cli "getblockcount"', shell = True))
    except:
...
while True:
    ...
    if firstTimeThrough == True:     # for the display, otherwise getinfo in the block waiting loop below
        try:    
            output = subprocess.check_output('/home/wallstreet/qtum-wallet/bin/qtum-cli "getinfo"', shell = True)
            # print(output)
        except:
    ...
    try:    
        output = subprocess.check_output('/home/wallstreet/qtum-wallet/bin/qtum-cli "getstakinginfo"', shell = True)
        # print(output)
    except:
        print("NO RESPONSE from qtumd. EXITING")
        sys.exit()
    ...
    immature = balance - weight                     # coins that will mature after 500 blocks

    # get netstakeweight in millions, the total of all QTUM currently being staked
    ...
    Left side:
    Unix time  | date       time     |    balance  |   stake  |    block
    1506651701 | 2017 09 28 22:21:41 |     1,000.0 |    500.0 |    17,430
    1506651701 | 2017 09 28 22:21:41 |    10,000.0 |  1,000.0 |   817,430
    1506651701 | 2017 09 28 22:21:41 | 1,123,000.0 | 58,000.0 | 5,817,430
                                      \pad1         \pad2      \pad3
    Right side:
    |     weight  | net weight | immature  | con | stking | exphrs  | secLast
    |     1,000.0 |  3,653,699 |     400.0 |   4 |  yes   | 9,340.0 |    10
    |    10,000.0 | 23,653,699 |   1,000.0 |  10 |  yes   |   340.0 |   258
    | 1,123,000.0 | 93,653,699 | 120,000.0 |  30 |  yes   |     2.3 |   558
    ...
    if stake <= 99999:
        ...
        pad2 = " " * (8 - len(stakeWithCommas))
        stakePadCommas = pad2 + stakeWithCommas
    else:
        stakePadCommas = "xxxxxxxx"
    ...
    if sendEmailForNewHour == False:  # avoids double print if coming through here for new hour 
        print(unixTimeFormatted, "|", current_time2, "|", balancePadCommas, "|", stakePadCommas, "|", blockPadCommas, "|", weightPadCommas, "|", netWeightPadCommas, "|", immaturePadCommas, "|", connectionsPad, "|", stakingForEmail, "|", exphrsPadCommas, "|", secLastPadCommas)  #PY3    
        # print unixTimeFormatted + " | " + current_time2 + " | " + balancePadCommas + " | " + stakePadCommas + " | " + blockPadCommas + " | " + weightPadCommas + " | " + netWeightPadCommas + " | " + immaturePadCommas + " | " + connectionsPad + " | " + stakingForEmail + " | " + exphrsPadCommas + " | " + secLastPadCommas  #PY2

    ''' typical display Skynet 9/29/2017

    Left side:      
    unix time  | date       time     |    balance  |   stake  |    block
    1506711943 | 2017 09 29 13:05:23 |     6,956.0 |    716.3 |    30,940
    1506712042 | 2017 09 29 13:07:52 |     6,956.0 |    716.3 |    30,941

    Right side:
   |     weight  | net weight | immature  | con | stking |  exphrs | secLast
   |     6,956.0 | 87,990,816 |       0.0 |  10 |   yes  |    36.0 |    22
   |     6,956.0 | 88,191,049 |       0.0 |  10 |   yes  |    36.1 |    92
    '''
    ...
    if logNewBlock == True:  # found a new block below, log it
        ...
        if enableLogging == True:  # for log (and Excel) no commas, Excel ingnores leading zeros
            tempStr = "100," + unixTimeFormatted +"," + current_time2 + "," + balanceFormatted + "," + stakeFormatted + "," + blockFormatted + "," + weightFormatted + "," + netStakeWeightFormatted + "," + immatureFormatted + "," + connectionsFormatted + "," + stakingForEmail + "," + expectedTimeHoursFormatted + "," + secondsSinceLastBlockFormatted + "," + savedLoopSeq
            if fileOpenQM == False:
                fileOpenQM = True
                outFileQM = open(out_file_name_QM, 'a')   # open log file
                # print("Opening file")
    ...
    if sendEmailForNewHour == True:
        ...
        if sendHourlyEmail == True:          # okay to send hourly status email
            tempMessage =  " bal " + balanceWithCommas + " blk " + blockWithCommas + " mwt " + weightWithCommas + " nwt " + netWeightWithCommas + stakingForEmail
    ...
    while breakOut == False:
        ...
        if block != oldBlock:  # found a new block
            ...
            if int((block) != int(oldBlock) + 1) and firstTimeThrough == False:
                ...
                if enableLogging == True:
                    now = datetime.now()
                    current_timeHMS = now.strftime("%H:%M:%S")
                    tempStr = "800, MISSED BLOCK, at," + current_timeHMS + ", before blk" + str(block) + ", seq" + loopSeq
                    if fileOpenQM == False:
                        fileOpenQM = True
        ...
        while timer() <= lastBlockCheckTime + delayDateHours:
            ...
            # catch the first five seconds of a new hour
            ...
            elif didNewHour == True and (unixTime % 3600) >= 15:
